...
bool Renderer::initialize()
{
	...
	if (!readImageFile(m_textureBuffer.GetAddressOf(),
		meta, L"src\\park.dds", true)) return false;
	...
	//定数バッファの生成とビューのセット
	{
		...
		if (!createConstBuffer(m_constBuffer.GetAddressOf(), &mat,
...
void Renderer::draw()
{
	...
	//angle += 1.0f;
	//*m_constBufferMap = XMMatrixRotationZ(XMConvertToRadians(60.0f));

	static float tx = 1.0f, vx = 0.01f;
	tx += vx;
	if (tx > 2.0f)
	{
		tx = 2.0f;
	...
	//*m_constBufferMap = XMMatrixTranslation(tx, 0.0f, 0.0f);
	*m_constBufferMap = XMMatrixScaling(tx, tx, tx);
